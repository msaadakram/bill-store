...
import {
  Zap, Hash, User, Download, Printer, RotateCcw,
  Calendar, Activity, MapPin, Clock, Copy, Check,
  Building2, BarChart3, ExternalLink,
} from "lucide-react";
import { motion } from "motion/react";
import type { BillDetails } from "../_lib/types";
import { AnimatedNumber } from "./AnimatedNumber";

const PITC_URL = "https://bill.pitc.com.pk/lescobill/general?refno=";

/** Full bill-result card with header, amount, breakdown, details grid, and actions */
export function BillResultCard({
  ...
  const statusMap = {
    ...
    unpaid:  { label: "Unpaid",  bg: "#FFF7ED", color: "#D97706", dot: "#F59E0B" },
  ...
  const items = [
    { icon: User,      label: "Customer Name",  value: bill.customerName,             color: "#3E8B8E" },
    { icon: Hash,      label: "Reference No.",   value: bill.referenceNo,              color: "#5A9FA2", copy: true },
    { icon: Building2, label: "Customer ID",     value: bill.customerId,               color: "#2D6A6D" },
    { icon: Activity,  label: "Meter No.",       value: bill.meterNo,                  color: "#059669" },
    { icon: Zap,       label: "Units Consumed",  value: `${bill.unitsConsumed} kWh`,   color: "#F59E0B" },
    { icon: BarChart3, label: "Connection Type", value: bill.connectionType,           color: "#7FB3B5" },
    { icon: Calendar,  label: "Bill Month",      value: bill.billMonth,                color: "#3E8B8E" },
    { icon: Clock,     label: "Due Date",        value: bill.dueDate,                  color: "#DC2626" },
  ];
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, #3E8B8E 0%, #5A9FA2 60%, #7FB3B5 100%)" }}
      >
        ...
        <div className="flex items-center gap-3 md:gap-4 relative">
          ...
          <div>
            <div className="text-white" style={{ fontSize: "15px", fontWeight: 800 }}>LESCO Bill Statement</div>
            <div className="text-teal-100" style={{ fontSize: "12px" }}>{bill.billMonth}</div>
          </div>
        </div>
        <div
          className="relative flex items-center gap-1.5 px-3 py-1.5 rounded-full border"
          style={{ backgroundColor: `${st.bg}20`, borderColor: `${st.dot}50`, color: "white" }}
        >
          <motion.div
      ...
      <div className="p-5 md:p-6 space-y-5">
        ...
        <motion.div
          ...
          style={{ background: "linear-gradient(135deg, #F0FAFB 0%, #D9F2F4 50%, #B3E5E9 100%)" }}
        >
          <div className="text-slate-500 mb-1" style={{ fontSize: "13px", fontWeight: 500 }}>Total Payable Amount</div>
          <div className="text-teal-700" style={{ fontSize: "clamp(32px, 8vw, 44px)", fontWeight: 900, lineHeight: 1.1 }}>
            PKR <AnimatedNumber value={bill.totalPayable} />
          </div>
          <div className="mt-1.5 text-red-500" style={{ fontSize: "13px", fontWeight: 600 }}>Due by {bill.dueDate}</div>
          <div className="mt-3 flex flex-wrap gap-2 text-xs justify-center">
            {breakdown.map((b, i) => (
              <div key={i} className="flex items-center gap-1">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: ["#3E8B8E", "#7FB3B5", "#F59E0B", "#DC2626"][i] }} />
                <span className="text-slate-500" style={{ fontSize: "11px" }}>{b.label}</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <div className="rounded-xl border border-slate-100 overflow-hidden">
          <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
            <span className="text-slate-600" style={{ fontSize: "12px", fontWeight: 700 }}>BILL BREAKDOWN</span>
          </div>
          {breakdown.map((row, i) => (
            <motion.div
              key={row.label}
              ...
              className={`flex items-center justify-between px-4 py-3 ${i < breakdown.length - 1 ? "border-b border-slate-50" : ""}`}
            >
              <span className="text-slate-500" style={{ fontSize: "13px" }}>{row.label}</span>
              <span style={{ fontSize: "13px", fontWeight: 600, color: row.color }}>PKR {row.value.toLocaleString()}</span>
            </motion.div>
          ))}
          <div
            className="flex items-center justify-between px-4 py-3 text-white"
            style={{ background: "linear-gradient(135deg, #2D6A6D 0%, #3E8B8E 100%)" }}
          >
            <span style={{ fontSize: "14px", fontWeight: 700 }}>Total Payable</span>
            <span style={{ fontSize: "16px", fontWeight: 800 }}>PKR {bill.totalPayable.toLocaleString()}</span>
          </div>
        </div>
          ...
          {items.map((item, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-slate-400" style={{ fontSize: "11px", fontWeight: 500 }}>{item.label}</div>
                <div className="text-slate-900 truncate" style={{ fontSize: "13px", fontWeight: 600 }}>{item.value}</div>
              </div>
              {item.copy && (
                <button
                  onClick={handleCopy}
        ...
        >
          ...
          <div>
            <div className="text-slate-400 mb-0.5" style={{ fontSize: "11px", fontWeight: 500 }}>Service Address</div>
            <div className="text-slate-900" style={{ fontSize: "13px", fontWeight: 600 }}>{bill.address}</div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="text-center pt-1">
          <button
            onClick={onReset}
