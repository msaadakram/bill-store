...
const FAKE_REF = "0621-7563-984210";

export function renderHowToPreview(activeStep: number, typedText: string): React.ReactNode {
  if (activeStep === 0)
    return (
      <div className="mt-2.5 flex items-center gap-2 px-3 py-2 rounded-xl border-2 border-amber-200/70 bg-amber-50/40">
        ...
        <div className="flex-1 font-mono text-slate-700 flex items-center overflow-hidden" style={{ fontSize: "11px", fontWeight: 600, letterSpacing: "0.8px" }}>
          ...
          <motion.span animate={{ opacity: [1, 0] }} transition={{ repeat: Infinity, duration: 0.55 }} className="inline-block w-[2px] h-3 bg-amber-600 ml-0.5 shrink-0 rounded-full" />
        </div>
      </div>
    );
  if (activeStep === 1)
    return (
      <div className="mt-2.5 flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-amber-50 border border-amber-100">
        <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 0.85, ease: "linear" }} className="w-4 h-4 rounded-full border-2 border-amber-100 border-t-amber-600 shrink-0" />
        <div className="flex gap-1 items-end">
          {[0, 1, 2].map((j) => (
            <motion.div key={j} className="w-1 rounded-full bg-amber-400" animate={{ height: ["4px", "10px", "4px"] }} transition={{ repeat: Infinity, duration: 0.75, delay: j * 0.18 }} />
          ))}
        </div>
        <span className="text-amber-600" style={{ fontSize: "10px", fontWeight: 600 }}>Connecting to GEPCO…</span>
      </div>
    );
  if (activeStep === 2)
    return (
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }} className="mt-2.5 rounded-xl overflow-hidden border border-emerald-100 shadow-md shadow-emerald-100/60">
        <div className="px-3 py-1.5 flex items-center justify-between" style={{ background: "linear-gradient(135deg, #059669, #10B981)" }}>
          <span className="text-white" style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "0.06em" }}>GEPCO BILL READY</span>
...
export const HOW_TO_STEPS = [
  { icon: Hash, title: "Enter Reference No.", desc: "Type your 14-digit reference number", color: "#92400E", color2: "#D97706", shadow: "rgba(146,64,14,0.30)" },
  { icon: Search, title: "Click Check Bill", desc: "Fetching your bill from GEPCO servers", color: "#0369A1", color2: "#0EA5E9", shadow: "rgba(14,165,233,0.28)" },
