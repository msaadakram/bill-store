...
const breakdownTemplate = [
  { label: "Bill Amount", color: "#0F172A" },
  { label: "GST (17%)", color: "#64748B" },
  { label: "Surcharge", color: "#64748B" },
  { label: "Previous Balance", color: "#DC2626" },
];

function buildItems(bill: BillDetails) {
  return [
    { icon: User, label: "Customer Name", value: bill.customerName, color: "#92400E" },
    { icon: Hash, label: "Reference No.", value: bill.referenceNo, color: "#D97706", copy: true },
    { icon: Building2, label: "Customer ID", value: bill.customerId, color: "#92400E" },
    ...
    { icon: BarChart3, label: "Connection Type", value: bill.connectionType, color: "#D97706" },
    { icon: Calendar, label: "Bill Month", value: bill.billMonth, color: "#92400E" },
    { icon: Clock, label: "Due Date", value: bill.dueDate, color: "#DC2626" },
  ];
}

function buildBreakdown(bill: BillDetails) {
  return [
    { ...breakdownTemplate[0], value: bill.billAmount },
    { ...breakdownTemplate[1], value: bill.taxAmount },
    { ...breakdownTemplate[2], value: bill.surcharge },
    { ...breakdownTemplate[3], value: bill.previousBalance },
  ];
}

function generateBillText(bill: BillDetails) {
  const lines = [
    "GEPCO BILL STATEMENT", "=".repeat(40),
    `Customer: ${bill.customerName}`, `Address: ${bill.address}`,
    `Reference No: ${bill.referenceNo}`, `Customer ID: ${bill.customerId}`,
    `Bill Month: ${bill.billMonth}`, `Meter No: ${bill.meterNo}`,
    `Connection: ${bill.connectionType}`, `Units Consumed: ${bill.unitsConsumed} kWh`,
    ...
    `Due Date: ${bill.dueDate}`, `Status: ${bill.status.toUpperCase()}`,
  ];
  return lines.join("\n");
}
...
export function BillResultCard({
  ...
  const breakdown = buildBreakdown(bill);
  ...
  const handleDownload = () => {
    const txt = generateBillText(bill);
    const blob = new Blob([txt], { type: "text/plain" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `GEPCO-Bill-${bill.referenceNo}.txt`;
    a.click();
  };
      ...
      >
        ...
        <div className="flex items-center gap-3 md:gap-4 relative">
          ...
          <div>
            ...
            >
              GEPCO Bill Statement
            </div>
            <div className="text-amber-200" style={{ fontSize: "12px" }}>
              {bill.billMonth}
            </div>
          </div>
        </div>
          ...
          {items.map((item, i) => (
            ...
            >
              ...
              {(item as any).copy && (
                <button
                  onClick={handleCopy}
                  className="shrink-0 w-7 h-7 rounded-lg flex items-center justify-center bg-white border border-slate-200 hover:border-amber-400 hover:bg-amber-50 transition-colors"
                >
                  {copied ? (
