  ...
  return (
    ...
    >
      ...
      <div className="p-6 space-y-5">
        <div className="h-28 bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl animate-pulse" />
        ...
        <div className="flex gap-3">
          <div className="flex-1 h-12 bg-amber-100 rounded-lg animate-pulse" />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/85 backdrop-blur-sm rounded-2xl">
        <div className="relative mb-4">
          <div className="w-20 h-20 rounded-full border-4 border-amber-100" />
          <motion.div
            animate={{ rotate: 360 }}
            transition={{ repeat: Infinity, duration: 1, ease: "linear" }}
            className="absolute inset-0 w-20 h-20 rounded-full border-4 border-transparent border-t-amber-600"
          />
          <div className="absolute inset-0 flex items-center justify-center">
            <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1 }}>
              <Zap className="w-7 h-7 text-amber-600" strokeWidth={2.5} />
            </motion.div>
          </div>
        </div>
        <p className="text-slate-700" style={{ fontSize: "15px", fontWeight: 600 }}>Fetching your bill…</p>
        <p className="text-slate-400 mt-1" style={{ fontSize: "13px" }}>Connecting to GEPCO servers</p>
