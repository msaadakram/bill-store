...
export function HowToCheckCard() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      i++;
      setTypedText("0721-4567-890123".slice(0, i));
      if (i >= "0721-4567-890123".length) clearInterval(t);
    }, 95);
    return () => clearInterval(t);
  }, [activeStep]);
    ...
    >
      <div className="relative overflow-hidden px-5 py-5" style={{ background: "linear-gradient(135deg, #1A1F2E 0%, #2C3444 60%, #3E4B5E 100%)" }}>
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border border-white/15" style={{ background: "linear-gradient(135deg, rgba(252,165,165,0.25), rgba(239,68,68,0.35))" }}>
      ...
      <div className="bg-white p-4">
        {HOW_TO_STEPS.map((step, i) => {
          ...
          return (
            <div key={i}>
              <motion.div animate={{ backgroundColor: isActive ? "#FFFBFB" : "#F8FAFC" }} transition={{ duration: 0.3 }} className="relative flex gap-3.5 rounded-xl p-3.5"
