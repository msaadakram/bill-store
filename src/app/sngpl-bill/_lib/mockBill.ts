...
/** Generate a randomised mock SNGPL gas bill for demo purposes */
export function mockBill(ref: string, mode: InputMode): BillDetails {
  ...
  const addresses = [
    "House #12-A, Street 7, Gulberg III, Lahore",
    "Plot 88, Model Town Extension, Lahore",
    "Bungalow 45, Sector F-11, Islamabad",
    "House #102, University Town, Peshawar",
    "Flat 3B, Jinnah Colony, Rawalpindi",
    "Shop #7, Saddar Bazaar, Peshawar",
  ];
  const conn = ["Residential", "Commercial", "Industrial", "Domestic (Protected)"];
  ...
  return {
    ...
    referenceNo: mode === "reference" ? ref : `0821${ref.padEnd(10, "0").slice(0, 10)}`,
    customerId: mode === "customer" ? ref : `SNGPL-${ref.slice(0, 6)}`,
    ...
    sanctionedLoad: `${r(5, 50)} MMBtu`,
    meterNo: `G${r(1000000, 9999999)}`,
    tarriffCode: ["Res-A", "Res-B", "Comm-A", "Ind-A"][r(0, 4)],
  };
}
