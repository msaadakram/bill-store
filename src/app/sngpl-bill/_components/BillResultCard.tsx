...
export function BillResultCard({
  ...
  const items = [
    ...
    { icon: Activity, label: "Meter No.", value: bill.meterNo, color: "#059669" },
    {
      icon: Flame,
      label: "Gas Consumed",
      value: `${bill.unitsConsumed} MMBtu`,
      color: "#D97706",
    },
    {
      icon: BarChart3,
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, #1D4ED8 0%, #3B82F6 60%, #93C5FD 100%)",
        }}
      >
        ...
        <div className="flex items-center gap-3 md:gap-4 relative">
          <div className="w-11 h-11 md:w-12 md:h-12 rounded-2xl bg-white/15 backdrop-blur-sm border border-white/20 flex items-center justify-center">
            <Flame className="w-5 h-5 md:w-6 md:h-6 text-orange-300" strokeWidth={2.5} />
          </div>
          <div>
            <div
            ...
            >
              SNGPL Bill Statement
            </div>
            <div className="text-blue-200" style={{ fontSize: "12px" }}>
              {bill.billMonth}
            </div>
          </div>
        </div>
          ...
          {items.map((item, i) => (
            ...
            >
              ...
              {(item as any).copy && (
                <button
                  onClick={handleCopy}
                  className="shrink-0 w-7 h-7 rounded-lg flex items-center justify-center bg-white border border-slate-200 hover:border-blue-400 hover:bg-blue-50 transition-colors"
                >
                  {copied ? (
