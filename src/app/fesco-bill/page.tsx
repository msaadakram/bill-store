...
import { mockBill } from "./_lib/mockBill";
import {
  WebPageJsonLd,
  SoftwareAppJsonLd,
  FaqJsonLd,
...
export default function FescoBillPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#F8FAFC" }}>
      <WebPageJsonLd
        url="https://billcheck.pk/fesco-bill"
        title="Check FESCO Bill Online 2026 — Instant FESCO Electricity Bill | BillCheck"
        description="Check your FESCO electricity bill online instantly. Enter your 14-digit reference number to view, download or print your FESCO bill. Free, fast, and secure. Serving Faisalabad & central Punjab."
      />
      <SoftwareAppJsonLd
        name="BillCheck — FESCO Bill Check"
        description="Check your FESCO electricity bill online instantly. Enter your 14-digit reference number to view, download or print your FESCO bill."
        url="https://billcheck.pk/fesco-bill"
      />
      <BreadcrumbJsonLd
        items={[
          ...
          { name: "FESCO Bill", url: "https://billcheck.pk/fesco-bill" },
        ]}
      />
      <OrganizationJsonLd companyId="fesco" />
      <FaqJsonLd faqs={faqs} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 lg:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <AnimatePresence mode="wait">
            ...
            {step === "error" && (
              <motion.div key="error" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="lg:col-span-2 bg-white rounded-2xl border border-red-100 shadow-lg p-6 sm:p-8 text-center">
                ...
                <button onClick={handleReset} className="px-5 py-2.5 rounded-xl text-white text-sm font-semibold transition-all hover:-translate-y-0.5" style={{ background: "linear-gradient(135deg, #7C3AED, #A78BFA)" }}>Try Again</button>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {/* ── Sidebar ── */}
          {step === "input" && (
            <FescoSidebar history={history} onLoad={loadFromHistory} onDelete={deleteHistory} onClear={clearHistory} />
          )}
        </div>
...
function FormCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden relative">
      <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-br from-purple-50 to-white pointer-events-none" />
      <div className="p-3 md:p-8 relative">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-5 md:mb-8">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl flex items-center justify-center shadow-lg shadow-purple-500/20 shrink-0 relative overflow-hidden group bg-gradient-to-br from-purple-600 to-purple-800">
            ...
            <div>
              <h2 className="text-slate-900" style={{ fontSize: "18px", fontWeight: 800, letterSpacing: "-0.01em" }}>Check FESCO Bill</h2>
          ...
          <div className="flex p-1 bg-slate-50/80 rounded-lg mb-4 md:mb-6 border border-slate-100">
            {(["reference", "customer"] as InputMode[]).map((m) => (
              <button key={m} onClick={() => switchMode(m)}
                className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 md:py-3.5 rounded-lg transition-all duration-300 ${mode === m ? "bg-white text-purple-700 shadow-sm shadow-slate-200/50" : "text-slate-400 hover:text-slate-600 hover:bg-slate-100/50"}`}
          ...
          <form onSubmit={handleSubmit} className="px-1 pb-1">
            <div className="relative group">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                {mode === "reference" ? <Hash className="w-5 h-5 text-slate-300 group-focus-within:text-purple-500 transition-colors" /> : <User className="w-5 h-5 text-slate-300 group-focus-within:text-purple-500 transition-colors" />}
              </div>
              <input ref={inputRef as React.RefObject<HTMLInputElement>} type="text" inputMode="numeric"
            ...
            <div className="mt-[-2px] h-1 w-[calc(100%-24px)] mx-auto bg-slate-100 rounded-b-lg overflow-hidden transition-opacity duration-300" style={{ opacity: inputValue.length > 0 ? 1 : 0 }}>
              <motion.div className="h-full rounded-full" style={{ background: progress === 100 ? "#10B981" : "linear-gradient(90deg, #7C3AED, #A78BFA)" }} animate={{ width: `${progress}%` }} transition={{ type: "spring", stiffness: 200, damping: 30 }} />
            </div>
            ...
            <motion.button type="submit" whileHover={{ scale: 1.01, y: -2 }} whileTap={{ scale: 0.98 }}
              className="w-full flex items-center justify-center gap-2 mt-4 px-5 py-3.5 rounded-lg text-white shadow-xl shadow-slate-400/20 hover:shadow-2xl hover:shadow-purple-500/20 transition-all"
