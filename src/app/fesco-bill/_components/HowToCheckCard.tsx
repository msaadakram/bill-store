...
export function HowToCheckCard() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      i++;
      setTypedText("0421-7563-984210".slice(0, i));
      if (i >= "0421-7563-984210".length) clearInterval(t);
    }, 95);
    return () => clearInterval(t);
  }, [activeStep]);
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden px-5 py-5" style={{ background: "linear-gradient(135deg, #1A1F2E 0%, #2C3444 60%, #3E4B5E 100%)" }}>
        <div className="absolute -top-6 -right-6 w-24 h-24 rounded-full" style={{ background: "radial-gradient(circle, rgba(167,139,250,0.18) 0%, transparent 70%)" }} />
        <div className="absolute -bottom-4 left-4 w-16 h-16 rounded-full" style={{ background: "radial-gradient(circle, rgba(139,92,246,0.15) 0%, transparent 70%)" }} />
        <motion.div className="absolute inset-0" style={{ backgroundImage: "linear-gradient(105deg, transparent 30%, rgba(167,139,250,0.08) 50%, transparent 70%)", backgroundSize: "200% 100%" }}
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border border-white/15" style={{ background: "linear-gradient(135deg, rgba(167,139,250,0.25), rgba(124,58,237,0.35))" }}>
      ...
      <div className="bg-white p-4">
        {HOW_TO_STEPS.map((step, i) => {
          ...
          return (
            <div key={i}>
              <motion.div animate={{ backgroundColor: isActive ? "#FAFAFE" : "#F8FAFC" }} transition={{ duration: 0.3 }} className="relative flex gap-3.5 rounded-xl p-3.5"
