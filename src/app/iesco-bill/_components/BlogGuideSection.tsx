    ...
    >
      ...
      <div className="text-center mb-10 md:mb-14">
        <div className="flex justify-center mb-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-teal-50 text-teal-700 border border-teal-100" style={{ fontSize: "12px", fontWeight: 700 }}>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
        <div className="h-1.5 w-full" style={{ background: "linear-gradient(90deg, #3E8B8E, #5A9FA2, #7FB3B5, #B2EBF2)" }} />
  ...
  return (
    <div className="max-w-3xl mb-10 pl-4 border-l-4 border-teal-400 bg-teal-50/30 rounded-r-xl py-4 pr-4">
      <p className="text-slate-600 italic" style={{ fontSize: "16px", lineHeight: 1.85 }}>
        IESCO — the <strong className="text-slate-800">Islamabad Electric Supply Company</strong> — is one of Pakistan&apos;s major electricity distribution companies, serving over <strong className="text-slate-800">3.5 million consumers</strong> across Islamabad and Rawalpindi districts. In the digital age, checking your electricity bill no longer requires standing in long queues or waiting for a physical copy to arrive in the post. This comprehensive guide walks you through every method available to check your IESCO bill — online, via SMS, through a mobile app, and even by phone.
      </p>
    </div>
  );
}
...
function TableOfContentsNav() {
  const items = [
    ...
    { id: "check-methods", label: "5 Ways to Check Your IESCO Bill" },
    { id: "iesco-duplicate-bill", label: "IESCO Duplicate Bill Guide" },
    { id: "tariff-rates", label: "IESCO Tariff Rates 2026" },
  ...
  return (
    <nav className="mb-10 rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white overflow-hidden">
      <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 bg-white">
        <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "linear-gradient(135deg, #3E8B8E, #5A9FA2)" }}>
      ...
      <div className="p-4 sm:p-5">
        <ol className="grid grid-cols-1 sm:grid-cols-2 gap-1">
          {items.map((item, i) => (
            <li key={item.id}>
              <a href={`#${item.id}`} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 hover:text-teal-700 hover:bg-teal-50 transition-all group">
                <span className="w-6 h-6 rounded-md flex items-center justify-center text-xs font-bold shrink-0 bg-slate-100 text-slate-500 group-hover:bg-teal-100 group-hover:text-teal-700 transition-colors">{i + 1}</span>
...
function WhyCheckSection() {
  ...
  return (
    <div id="why-check" className="mb-10 scroll-mt-24">
      <SectionHeading icon={Shield} iconBg="linear-gradient(135deg, #E0F7FA, #B2EBF2)" iconColor="text-teal-700" title="Why You Should Check Your IESCO Bill Regularly" />
      <p className="text-slate-600 mb-4" style={{ fontSize: "15px", lineHeight: 1.85 }}>
        Regular bill checking is more than just a financial habit — it is a smart utility management practice. By monitoring your IESCO electricity bill each month, you can catch billing errors well before the due date, track your household or business consumption trends, plan your budget more effectively, and avoid late payment surcharges. IESCO bills include several components beyond the basic unit price — such as tariff adjustments, Fuel Price Adjustment (FPA), and government taxes.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-5">
        {benefits.map((text, i) => (
          <div key={i} className="flex items-start gap-3 p-3.5 rounded-xl bg-slate-50 border border-slate-100">
            <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5" style={{ background: "linear-gradient(135deg, #3E8B8E, #5A9FA2)" }}><CheckCircle2 className="w-3 h-3 text-white" strokeWidth={3} /></div>
...
function BillInfoSection() {
  ...
  return (
    <div id="bill-info" className="mb-10 scroll-mt-24">
      <SectionHeading icon={Info} iconBg="linear-gradient(135deg, #FFF7ED, #FEE2D5)" iconColor="text-orange-600" title="What Information Is Printed on Your IESCO Bill" />
      <p className="text-slate-600 mb-5" style={{ fontSize: "15px", lineHeight: 1.85 }}>
        Before you dive into how to check your bill, it is worth knowing what you are looking for. Your IESCO electricity bill contains several key data points. The most important is your <strong className="text-slate-800">14-digit Reference Number</strong> (also called Consumer Number), printed prominently at the top of every bill.
      </p>
  ...
  return (
    <div id="check-methods" className="mb-8 scroll-mt-24">
      <div className="flex items-center gap-3 mb-2">
        <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: "linear-gradient(135deg, #E0F7FA, #B2EBF2)" }}><Activity className="w-4 h-4 text-teal-700" strokeWidth={2.5} /></div>
        <h3 className="text-slate-900 font-serif" style={{ fontSize: "20px", fontWeight: 800, letterSpacing: "-0.01em" }}>5 Ways to Check Your IESCO Bill</h3>
      </div>
      ...
      <MethodCard num="1" heading="Check via BillCheck Online Portal (Fastest Method)" bg="bg-teal-50">
        <p className="text-slate-600 mb-4" style={{ fontSize: "15px", lineHeight: 1.85 }}>The easiest way to check your IESCO bill is through an online platform like <strong className="text-slate-800">BillCheck</strong>, which fetches real-time data directly from IESCO servers:</p>
        <div className="space-y-3">
          {[
            "Open your browser and visit the BillCheck IESCO Bill page (you are already here!)",
            ...
            <div key={i} className="flex items-start gap-3">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 text-teal-700" style={{ background: "linear-gradient(135deg, #E0F7FA, #B2EBF2)", fontSize: "11px", fontWeight: 800 }}>{String(i + 1).padStart(2, "0")}</div>
...
function MethodCard({ num, heading, bg, children }: { num: string; heading: string; bg: string; children: React.ReactNode }) {
  return (
    <div className="mb-8 rounded-xl border border-slate-100 overflow-hidden" style={{ borderLeft: "4px solid #3E8B8E" }}>
      <div className={`flex items-center gap-3 px-5 py-4 ${bg}`}>
        <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white shrink-0" style={{ background: "linear-gradient(135deg, #3E8B8E, #5A9FA2)", fontSize: "13px", fontWeight: 800 }}>{num}</div>
...
function IescoDuplicateBillSection() {
  return (
    <div id="iesco-duplicate-bill" className="mb-10 scroll-mt-24">
      <SectionHeading icon={Activity} iconBg="linear-gradient(135deg, #CCFBF1, #99F6E4)" iconColor="text-teal-700" title="How to Get a Duplicate IESCO Bill Online" />
      <p className="text-slate-600 mb-4" style={{ fontSize: "15px", lineHeight: 1.85 }}>
        Lost your original IESCO bill or need a second copy? Getting a <strong className="text-slate-800">duplicate IESCO bill</strong> is straightforward. Many consumers in Islamabad and Rawalpindi need a <strong className="text-slate-800">duplicate bill IESCO</strong> copy for payment records, tax filing, or when the physical copy is misplaced. All you need is your <strong className="text-slate-800">14-digit reference number</strong> (also called your consumer number), printed on any previous IESCO electricity bill.
      </p>
      <p className="text-slate-600 mb-5" style={{ fontSize: "15px", lineHeight: 1.85 }}>
        A duplicate IESCO bill contains exactly the same information as the original: your reference number, units consumed in kWh, bill amount with NEPRA tariff breakdown, due date, and all surcharges. Follow these steps to get your duplicate IESCO bill:
      </p>
      <div className="space-y-3 mb-5">
        {[
          "Locate your 14-digit reference number from any previous IESCO bill or SMS from 8118.",
          "Enter the reference number in the IESCO Bill Check tool at the top of this page.",
          "Click Check Bill Now — your current duplicate bill loads in seconds with units consumed (kWh), bill amount, and due date.",
          "View the full breakdown including NEPRA tariff charges, FPA, GST, and any previous balance.",
          "Download or print a PDF copy of your duplicate IESCO bill for your records.",
        ].map((step, i) => (
          <div key={i} className="flex items-start gap-3">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 text-teal-700" style={{ background: "linear-gradient(135deg, #CCFBF1, #99F6E4)", fontSize: "11px", fontWeight: 800 }}>{String(i + 1).padStart(2, "0")}</div>
            <p className="text-slate-600 pt-0.5" style={{ fontSize: "14px", lineHeight: 1.7 }}>{step}</p>
          </div>
        ))}
      </div>
      <div className="flex items-start gap-2.5 p-4 rounded-xl bg-teal-50 border border-teal-100">
        <Star className="w-4 h-4 text-teal-600 shrink-0 mt-0.5" strokeWidth={2.5} />
        <p className="text-teal-700" style={{ fontSize: "13px", lineHeight: 1.6 }}><strong>Important:</strong> IESCO bills are issued monthly. Your duplicate bill will show the most recent billing cycle&apos;s units consumed (kWh), bill amount, and due date. If you need an older duplicate bill, visit your nearest IESCO subdivision office in Islamabad or Rawalpindi.</p>
      </div>
    </div>
  );
}
...
function TariffRatesSection() {
  const rows = [
    ...
    { slab: "2", units: "101 – 200", rate: "11.56", est: "≈ PKR 1,930", cat: "Low Use", color: "#3E8B8E" },
    { slab: "3", units: "201 – 300", rate: "16.60", est: "≈ PKR 3,590", cat: "Moderate", color: "#0369A1" },
    { slab: "4", units: "301 – 400", rate: "22.65", est: "≈ PKR 5,855", cat: "Standard", color: "#3E8B8E" },
    ...
    { slab: "8", units: "700+", rate: "37.75", est: "≈ PKR 19,000+", cat: "Peak", color: "#991B1B" },
  ];
  return (
    <div id="tariff-rates" className="mb-10 scroll-mt-24">
      <SectionHeading icon={BarChart3} iconBg="linear-gradient(135deg, #E0F7FA, #B2EBF2)" iconColor="text-teal-600" title="IESCO Electricity Tariff Rates 2026" />
      <p className="text-slate-600 mb-5" style={{ fontSize: "15px", lineHeight: 1.85 }}>IESCO uses a progressive slab-based tariff structure for residential consumers set by NEPRA. The more units you consume, the higher the per-unit rate.</p>
      <div className="rounded-xl border border-slate-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[480px]">
            <thead>
              <tr style={{ background: "linear-gradient(135deg, #3E8B8E, #5A9FA2)" }}>
            ...
            <tbody>
              {rows.map((row, i) => (
                <tr key={row.slab} className={`border-t border-slate-100 transition-colors hover:bg-teal-50/40 ${i % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
      ...
      <div className="mt-5 flex items-start gap-2.5 p-4 rounded-xl bg-teal-50 border border-teal-100">
        <Star className="w-4 h-4 text-teal-600 shrink-0 mt-0.5" strokeWidth={2.5} />
        <p className="text-teal-700" style={{ fontSize: "13px", lineHeight: 1.6 }}><strong>Tip:</strong> Reducing just 50–60 units can drop you into a lower slab and save PKR 1,000–2,000 per month.</p>
      </div>
    </div>
  );
}
...
function BillComponentsSection() {
  ...
  return (
    <div id="bill-components" className="mb-10 scroll-mt-24">
      <SectionHeading icon={BarChart3} iconBg="linear-gradient(135deg, #FFF7ED, #FEE2D5)" iconColor="text-orange-600" title="Understanding Your IESCO Bill Components" />
      <p className="text-slate-600 mb-5" style={{ fontSize: "15px", lineHeight: 1.85 }}>When you check your IESCO bill, you will notice several line items that together make up the total payable amount. Here is a plain-language breakdown:</p>
      <div className="space-y-3">
        {items.map((item) => (
...
function BillErrorsSection() {
  return (
    <div id="bill-errors" className="mb-10 scroll-mt-24">
      ...
      <p className="text-slate-600 mb-4" style={{ fontSize: "15px", lineHeight: 1.85 }}>
        If you notice a discrepancy — incorrect meter reading, unexpected spike in units, wrong tariff category, or unexplained charge — visit your nearest IESCO subdivision office in Islamabad or Rawalpindi with your CNIC and a copy of the bill. Submit a written complaint at the customer service counter. Alternatively, call <strong className="text-slate-800">118</strong> or file a complaint through the IESCO consumer portal. Under NEPRA regulations, IESCO must respond within <strong className="text-slate-800">15 working days</strong>.
      </p>
    </div>
  );
}
...
function ReduceBillSection() {
  const tips = [
    ...
    "Consider rooftop solar panels — IESCO&apos;s net metering policy lets you export surplus energy to offset your bill.",
  ];
  return (
    <div id="reduce-bill" className="mb-8 scroll-mt-24">
      <SectionHeading icon={TrendingUp} iconBg="linear-gradient(135deg, #ECFDF5, #D1FAE5)" iconColor="text-emerald-600" title="Practical Tips to Reduce Your IESCO Bill" />
...
function ConclusionBlock() {
  return (
    <div className="rounded-xl p-6 md:p-8 border-l-4 border-teal-500" style={{ background: "linear-gradient(135deg, #F0F9FF 0%, #E0F7FA 50%, #B2EBF2 100%)" }}>
      ...
      <p className="text-teal-800 italic" style={{ fontSize: "15px", lineHeight: 1.85 }}>
        Checking your IESCO bill online has never been more accessible. Whether you prefer the instant convenience of <strong>BillCheck</strong>, the official IESCO portal, the no-internet simplicity of SMS to 8118, the mobile app, or the free helpline on 118 — multiple reliable methods are available. Make bill checking a regular monthly habit: verify your charges, monitor consumption trends, and act immediately on any discrepancy you spot. Bookmark this page and use the IESCO Bill Check tool at the top of this page to fetch your latest bill right now.
      </p>
    </div>
  );
}
